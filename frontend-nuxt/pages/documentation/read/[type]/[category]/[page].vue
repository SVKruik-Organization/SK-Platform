...
// Watchers
watch(useRoute(), async (from: RouteLocation, to: RouteLocation): Promise<void> => {
    if (from.path !== to.path) await loadContent();
});
 ...
 */
async function loadContent(): Promise<void | NavigationFailure | undefined> {
    if (!props.type || !props.category) return;
    ...
    if (!props.page) {
        if (!documentationStore.validateFolder(props.category, props.type)) return useRouter().push(`/documentation/notfound?type=${props.type}&category=${props.category}`);
        fileData.value = await useFetchDocumentationDefault(props.category, documentationStore.version, documentationStore.language, props.type).value;

        // Specific Documentation Page
    } else {
        if (!documentationStore.validatePage(props.category, props.page, props.type)) return useRouter().push(`/documentation/notfound?type=${props.type}&category=${props.category}&page=${props.page}`);
        fileData.value = await useFetchDocumentationPage(props.category, props.page, documentationStore.version, documentationStore.language, props.type).value;
        if (typeof fileData.value === "boolean") return;
        relatedItems.value = fileData.value.related;

        // Chapter URL Hash
        if (useRoute().hash) setTimeout(() => {
            scrollAnchor();
        }, 300);
...
function scrollAnchor(): void {
    if (typeof fileData.value === "object") {
        ...
        if (!chapters.includes(useRoute().hash)) return;
        const element: HTMLAnchorElement | null = document.getElementById(useRoute().hash.slice(1)) as HTMLAnchorElement | null;
        const chapterMarker: HTMLAnchorElement | null = document.getElementById(`${useRoute().hash.slice(1)}_aside`) as HTMLAnchorElement | null;
